...
typedef struct{
  ...
  uint8_t           cache_retry;
  uint8_t           cache[(ATT_MTU_SIZE-3) * 16];
}otam_fw_t;
...
static int load_data_cache(void)
{
  ...
  uint16_t size = mtu_a * 16;

  if(pfw->offset + size > ppart->size){
    size = ppart->size - pfw->offset;
  }

  memset(pfw->cache, 0, (ATT_MTU_SIZE-3)* 16);
  memcpy(pfw->cache, (uint8_t*)(ppart->flash_addr + pfw->offset), size);
  pfw->cache_size = size;
  pfw->cache_offset = 0;
  pfw->cache_retry = 0;
...
#define OTAM_FW_DATA_ADDR   0x11032000
...
int otamProtocol_app_start_ota(uint8_t mode)
{
  ...
  if(method->write_cmd){
    ...
    return method->write_cmd(data, 2, 0);
  }
  return PPlus_ERR_NOT_REGISTED;
}
