...
static void pin_event_handler(GPIO_Pin_e pin, IO_Wakeup_Pol_e type)
{
	...
	if(i < KEY_NUM){
		printf("=========> key state: %d", key_state.key[i].state);
...
//				key_press_debonce_handler(i,type);
                osal_set_event(key_state.task_id, BLEMESH_KEY_PRESS_EVT);
				break;
...
void dongleKey_init(uint8 task_id, key_callback_hdl_t keyCb)
{
	uint8 i;
	
    key_state.key[0].pin = GPIO_P14;
	key_state.key[0].state = STATE_KEY_IDLE;
    key_state.key[0].idle_level = HIGH_IDLE;  
    key_state.task_id = task_id;
    
    key_state.key_callbank = keyCb;
	...
	for(i = 0; i < KEY_NUM; ++i){
		if(key_state.key[i].idle_level == LOW_IDLE){
			...
			hal_gpio_pull_set(key_state.key[i].pin, STRONG_PULL_UP);
		}
