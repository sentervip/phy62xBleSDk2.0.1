...
{
  osal_start_timerEx(ota_TaskID, OTA_TIMER_EVT, (uint32)timeout);
}
static void stop_timer(void)
{
  osal_clear_event(ota_TaskID, OTA_TIMER_EVT);
  osal_stop_timerEx(ota_TaskID, OTA_TIMER_EVT);
}
...
static int write_app_boot_sector(void)
{
  ...
  for(idx = 0; idx < s_ota_ctx.part_num; idx ++){
    bs[4+ idx *4]       = s_ota_ctx.part[idx].flash_addr;
    bs[4+ idx *4 + 1]   = s_ota_ctx.part[idx].run_addr;
    bs[4+ idx *4 + 2]   = s_ota_ctx.part[idx].size;
    bs[4+ idx *4 + 3]   = (uint32_t)s_ota_ctx.part[idx].checksum;
  }
    
  ret = ota_flash_write_boot_sector(bs, (s_ota_ctx.part_num*4 +4)*4);
  return ret;
}
